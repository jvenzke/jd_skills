...
- **Project intake (optional)**: Do **not** search for a parent project. Attach only when (1) the user links `.working_items/{project}/scope.md` or `steps/{NN}-*.md`, or names `{project}` and that `scope.md` exists, or (2) a `phase-*/tasks.md` for this `{task}` already has `project:`. Otherwise skip this block (behavior unchanged).
  When attached: read `{project}/scope.md` (including `## Deep modules`; insert that heading after `## Scope` if missing — fill from existing quality/deepening bullets or `- none`, no full rewrite), the step file (linked step, or next `- [ ]`/`- [o]` on the tracker if only `scope.md` was named), and `{project}/agent_notes.md`. Treat the **step** as the request; still research, clarify, and write phase/implementation plans as usual. Honor in-work deepening named for this step. Do not invent a different project. If creating a new task folder, `{task}` = `{project}--{step-stem}` (step filename without `.md`). Set that tracker line to `- [o]` if it was `- [ ]`. On resume, include project tracker status in the chat summary.
...
- **Project report-back (optional)**: If a parent project is attached, write material tradeoffs and work pushed out of this step onto `{project}/scope.md` (`## Tradeoffs / push-outs`, `## Later`, later tracker steps / new last step). If deepening was dropped or deferred, patch `## Deep modules` (insert that heading after `## Scope` if missing). Prefer deferring widening work over expanding this step. Pause if that changes approved project outcomes, owning boundaries, or drops/replaces a step. If no parent is attached, skip.
