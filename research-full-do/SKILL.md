...
- **Review Artifacts:** Create `review_artifacts/` inside the task folder for files that should feed `RESEARCH_REVIEW.html`. Enforce a **metadata-only** approach for data artifacts to minimize token usage: output summary statistics (e.g., `df.describe()`, row counts) and schema definitions. Exclude raw CSV/JSON rows from LLM context. Keep artifacts compact: summary statistics, schema or data dictionary notes, final SQL, final Python snippets or script paths, standalone Plotly HTML for reviewable charts, optional Plotly PNG previews only for Markdown/static-preview use, and a manifest named `artifact_manifest.json`.
